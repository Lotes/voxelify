...
/**
 * Runs the `slice3d` command with the given arguments.
 * @param {Array<String>} args the process arguments from command line
 * @returns {void}
 */
function run (args) {
  /**
   * Helper function to display errors and exit process.
  ...
  if (sizeStruct.min === null && sizeStruct.max === null) {
    error('Minimal or maximal size must be given!')
    return
  } else if (sizeStruct.min !== null && sizeStruct.max !== null) {
    error('Only one size (not both, min and max) must be given!')
    return
  }
