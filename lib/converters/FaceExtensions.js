...
module.exports.split = function * (face, plane) {
  function returnItem (newFace, isFront) {
    return {
      item: newFace,
      isFront: isFront
    }
  ...
  switch (polygonType) {
    case Classification.Coplanar:
      yield returnItem(face, plane.normal.dot(face.normal) > 0)
      break
    case Classification.Front:
      yield returnItem(face, true)
      break
    case Classification.Back:
      yield returnItem(face, false)
      break
    case Classification.Spanning:
      ...
      if (front.length >= 3) {
        yield returnItem(new Face(front[0], front[1], front[2], face.material), true)
        if (front.length === 4) {
          yield returnItem(new Face(front[2], front[3], front[0], face.material), true)
        }
      }
      if (back.length >= 3) {
        yield returnItem(new Face(back[0], back[1], back[2], face.material), false)
        if (back.length === 4) {
          yield returnItem(new Face(back[2], back[3], back[0], face.material), false)
        }
      }
      break
  }
