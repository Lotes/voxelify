  ...
  return Rx.Observable.create(observer => {
    ...
    for (var y = v1.y; y <= v2.y; y++) {
      ...
      for (var x = minX; x <= maxX; x++)
        observer.onNext(new Vector2(x, y))
      x1 += slope1
      x2 += slope2
    }
...
//returns: Observable<Vector2> with all pixel coordinates in the given triangle
module.exports.rasterize = function(userTriangle) {
  ...
  /* check for trivial case of top-flat triangle */
    ...
    return fillTopFlatTriangle([v1, v2, v3], true)
  else {
    ...
    return Rx.Observable.concat(
      fillBottomFlatTriangle([v1, v2, vTmp]),
      fillTopFlatTriangle([v2, vTmp, v3], false)
    )
  }
}
