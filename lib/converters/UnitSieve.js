...
class Node {
   ...
   * @param {Function} split the splitter function taking (obj, plane)
   * @returns {Generator<Object>} returns all splitted objects on leaf level
   */
  * add (item, split) {
    for (let x of split(item, this.plane)) {
      if (x.isFront) {
...
class Leaf {
   ...
   * @returns {Generator<Object>} returns `item`
   */
  * add (item) {
    yield {
      item: item,
...
class UnitSieve {
   ...
   * @param {Function} split is the split function of type Function(Item, Plane): Observable<{ item: Item, isFront: boolean }>
   * @returns {Generator<Object>} a set of splitted objects, splitted by the sieve
   */
