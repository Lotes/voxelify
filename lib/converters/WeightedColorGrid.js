...
class WeightedColorGrid {
  ...
  add (point, color) {
    ...
    if (cell === null) {
      cell = this._cells[index] = new WeightedColor()
    }
    cell.add(color)
  }
  // param point is THREE.Vector3
  ...
  // param coord is a THREE.Vector3 with integers
  floodDelete (coord) {
    const queue = [coord]
    while (queue.length > 0) {
      let point = queue.pop()
      let index = this._getIndex(point)
      ...
      FLOOD_DIRECTIONS.forEach((dir) => {
        ...
        if (this._bbox.containsPoint(newPoint)) {
          queue.push(newPoint)
        }
      })
    }
  ...
  save (fileName) {
    ...
    this.forEach(function (x, y, z, color) {
      ...
      if (color.weight > 0) {
        ...
        data[indexX4] = data[indexX4 + 1] = data[indexX4 + 2] = 0
        data[indexX4 + 3] = 0x80
      }
    })
