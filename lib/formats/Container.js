        ...
        let promises = Object.keys(zip.files).map(fileName => {
          ...
          if (resultPng) {
            ...
            return zip.file(fileName)
              ...
              .then(image => {
                let imageData = utils.convertImageToPixelData(image)
                let data = imageData.data
                let sprite = this.addSprite(key)
                let pixelCount = image.width * image.height
                for (let index = 0; index < 4 * pixelCount; index += 4) {
                  let color = utils.pixelToColor(data, index)
                  if (color !== 0) {
                    const quarterIndex = Math.floor(index / 4)
                    const x = quarterIndex % image.width
                    const y = Math.floor(quarterIndex / image.width)
