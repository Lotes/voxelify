 ...
 * @returns {Function} null or the content of 'toMesh.js'
 */
function scanFiles (rootPath, fileNames, validators, migratorMap) {
  fileNames.forEach(fn => {
    // validators
  ...
  try {
    return require(path.join(rootPath, 'toMesh.js'))
  } catch (err) {
    return null
  }
}
...
scanFiles(__dirname, rootDir.files, FormatCollection.Validators, null)
...
rootDir.folders.forEach(folder => {
  ...
  scanFiles(fullFolderPath, formatDir.files, Format.Validators, null)
  formatDir.folders.forEach(version => {
    ...
    const toMesh = scanFiles(fullVersionFolderPath, versionDir.files, versionValidators, migratorMap)
    Format.Versions.set(version, {
      Validators: versionValidators,
      Migrators: migratorMap,
      toMesh: toMesh || function () {
...
function checkContainer (container) {
  ...
  let validators = rootValidators.concat(formatValidators, versionValidators)
...
/**
 * Converts a container depending on the format into a mesh
 * @param {Container} container the container with an individual format
 * @returns {THREE.Object3D} the 3D object
 */
function toMesh (container) {
  ...
  const Version = Format.Versions.get(version)
  return Version.toMesh(container)
}
